...
static int screenshotI = 0;
static int screenshotNum = 455;//9.1 * 50
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
		...
		{ //set properties
			...
			double curTime{ lastTime + 20.0 };//std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - completionTime).count() / 1000.0 };
		...
		if(takeScreenshot) {
            ...
            auto a = std::to_string(++screenshotI);
            a = std::string(6 - std::min<int>(6, a.length()), '0') + a;
            stbi_write_png(
                (std::string("./sc/screenshot") + a + ".png").c_str(),
                windowSize.x,
                windowSize.y,
                3,
                data + 3 * windowSize.x * (windowSize.y - 1),
                -windowSize.x * 3
            );
